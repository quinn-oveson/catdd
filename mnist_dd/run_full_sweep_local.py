...
This is much more expensive than run_stage1_local.py: each task_id trains one
weight-reuse "segment" of H's -- some are a single H (fixed 6000 epochs, no
early stopping), others chain several H's sequentially (weight reuse
requires the order) -- see full_sweep.py's module docstring for how segments
are derived from sweep_config's REUSE_WEIGHTS_* flags. Consider running a
single task_id locally first to get a feel for timing before looping over
all of them.
...
def main():
    ...
    for task_id in range(TOTAL_TASKS):
        ...
        print(f"[{task_id + 1}/{TOTAL_TASKS}] kind={kind}, lr={spec.lr}, batch_size={spec.batch_size}, "
              f"seed={spec.seed}, H_list={spec.H_list} (running --full_batch locally)")
